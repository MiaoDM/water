...
u8 HC05_Init(void)
{
  ...
  RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOA, ENABLE);//使能GPIOA,GPIOF时钟
 
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_8; //LED对应引脚
  ...
  GPIO_Init(GPIOA, &GPIO_InitStructure); //根据设定参数初始化
	...
	uart5_init(9600);	//初始化串口3为:9600,波特率.
	...
	while(retry--)
	{
		...
		if(UART5_RX_STA&0X8000)	//接收到一次数据了
		{
			...
			UART5_RX_STA=0;			 
			if(temp==4&&UART5_RX_BUF[0]=='O'&&UART5_RX_BUF[1]=='K')
			{
				temp=0;//接收到OK响应
				break;
			}
...
{	 		    
	...
	while(retry--)
	{
		...
		if(UART5_RX_STA&0X8000)	//接收到一次数据了
		{
			temp=UART5_RX_STA&0X7FFF;	//得到数据长度
			UART5_RX_STA=0;			 
			if(temp==13&&UART5_RX_BUF[0]=='+')//接收到正确的应答了
			{
				temp=UART5_RX_BUF[6]-'0';//得到主从模式值
				break;
			}
		}		
	}
...
{	 		    
	...
	while(retry--)
	{
		...
		if(UART5_RX_STA&0X8000)	//接收到一次数据了
		{
			temp=UART5_RX_STA&0X7FFF;	//得到数据长度
			UART5_RX_STA=0;			 
			if(temp==4&&UART5_RX_BUF[0]=='O')//接收到正确的应答了
			{			
				temp=0;
				break;			 
			}
